...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    lines = [
        "#!/bin/bash",
        f"#SBATCH --job-name=downsample_prec",
        f"#SBATCH --time={time}",
    ...
    # cfg_path = global_cfgs.get("instance_segmentation/downsample", "kns/instance_segmentation/configs/config_downsample.yaml")
    cfg_path = (
        global_cfgs.get("instance_segmentation", {})
                .get("downsample", "kns/instance_segmentation/configs/config_downsample.yaml")
    )
    lines += [
        f"{python_bin} -m kns.instance_segmentation.tools.downsample_prec "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    job_dir = hpc.get("job_dir", "./jobs/downsample")
...
def downsample_prec_hpc(global_cfg, restart=False, dry_run=False):
