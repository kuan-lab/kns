...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    lines = [
        "#!/bin/bash",
        f"#SBATCH --job-name=convert_prec",
        f"#SBATCH --time={time}",
    ...
    # cfg_path = global_cfgs.get("instance_segmentation/mask_tif", "kns/instance_segmentation/configs/config_mask_tif.yaml")
    cfg_path = (
        global_cfgs.get("instance_segmentation", {})
                .get("mask_tif", "kns/instance_segmentation/configs/config_mask_tif.yaml")
    )
    lines += [
        f"{python_bin} -m kns.instance_segmentation.tools.mask_tif "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using instance_segmentation.tools.mask_tif.
    """
    ...
    job_dir = hpc.get("job_dir", "./jobs/mask_tif")
...
def mask_tif_hpc(global_cfg, restart=False, dry_run=False):
