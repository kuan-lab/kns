...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    # cfg_path = global_cfgs.get("instance_segmentation/resize_tif", "kns/instance_segmentation/configs/config_resize_tif.yaml")
    cfg_path = (
        global_cfgs.get("instance_segmentation", {})
                .get("resize_tif", "kns/instance_segmentation/configs/config_resize_tif.yaml")
    )
    lines += [
        f"{python_bin} -m kns.instance_segmentation.tools.resize_tif "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using instance_segmentation.tools.resize_tif.
    """
    ...
    job_dir = hpc.get("job_dir", "./jobs/resize_tif")
...
def resize_tif_hpc(global_cfg, restart=False, dry_run=False):
