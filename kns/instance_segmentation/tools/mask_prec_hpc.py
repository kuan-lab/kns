...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    # cfg_path = global_cfgs.get("instance_segmentation/mask_prec", "kns/instance_segmentation/configs/config_mask.yaml")
    cfg_path = (
        global_cfgs.get("instance_segmentation", {})
                .get("mask_prec", "kns/instance_segmentation/configs/config_mask.yaml")
    )
    lines += [
        f"{python_bin} -m kns.instance_segmentation.tools.mask_prec "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using instance_segmentation.tools.mask_prec.
    """
