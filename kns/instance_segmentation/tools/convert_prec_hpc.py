...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    lines += [
        f"{python_bin} -m kns.instance_segmentation.tools.convert_prec "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    Process a set of block indices locally on nodes using instance_segmentation.tools.convert_prec.
    """
    ...
    job_dir = hpc.get("job_dir", "./jobs/convert")
...
def convert_prec_hpc(global_cfg, restart=False, dry_run=False):
