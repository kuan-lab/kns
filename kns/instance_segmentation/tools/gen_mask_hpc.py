...
def _slurm_script(cfg, stage_cfg, job_dir, array_len):
    ...
    lines = [
        "#!/bin/bash",
        f"#SBATCH --job-name=gen_aff_mask",
        f"#SBATCH --time={time}",
    ...
    # cfg_path = global_cfgs.get("instance_segmentation/gen_mask", "kns/instance_segmentation/configs/config_gen_mask.yaml")
    cfg_path = (
        global_cfgs.get("instance_segmentation", {})
                .get("gen_mask", "kns/instance_segmentation/configs/config_gen_mask.yaml")
    )
    lines += [
        f"{python_bin} -m kns.instance_segmentation.tools.gen_mask "
        f"--config {cfg_path}"
    ]
...
def submit_local_hpc(global_cfg, stage_cfg, restart=False, dry_run=False):
    ...
    job_dir = hpc.get("job_dir", "./jobs/gen_mask")
...
def gen_aff_mask_hpc(global_cfg, restart=False, dry_run=False):
