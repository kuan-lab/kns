...
def _default_config_path() -> Path:
    ...
    return Path(__file__).with_name("kns/instance_segmentation/configs/config.yaml")

def _default_global_config_path() -> Path:
    # config.yaml in the configs directory
    return Path(__file__).with_name("kns/config.yaml")
...
def load_global_config_path(path: str = None):
    ...
    if path is None:
        ...
        if not cfg_path.is_file():
            # Allow input similar to “kns/config.yaml”"
