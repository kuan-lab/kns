...
# ==== Modules ====
import kns.instance_segmentation as ins_segmentation
import kns.pytorch_connectomics as aff_inference
...
def show_environment_info(config_path):
    ...
    console.print(f"[bold white] Tool:[/bold white] Kuan Lab Neuron Segmentation CLI")
...
# ---------------------------------------------------
def show_menu():
    console.rule("[bold bright_white]Kuan Lab Neuron Segmentation CLI[/bold bright_white]", style="bold white")
    table = Table(
        box=box.SIMPLE,
    ...
    table.add_row("1", "Affinity Map Inference", "Train / Infer affinity maps using deep learning models")
    table.add_row("2", "Instance Segmentation", "Perform block-based segmentation and merge across tiles")
    table.add_row("0", "Exit", "Close the CLI interface")
    console.print(table)
...
def main():
    parser = argparse.ArgumentParser(
        prog="Neuron Segmentation Pipeline",
        description="Unified CLI for Kuan Lab's segmentation workflows",
    )
    ...
    if len(sys.argv) == 1:
        ...
        while True:
            show_menu()
            choice = Prompt.ask("\n> Select a module", choices=["0", "1", "2"], default="0")

            if choice == "1":
                console.rule("[bold bright_white]Affinity Map Inference Module[/bold bright_white]", style="bold white")
                aff_inference.run_interactive()

            elif choice == "2":
                console.rule("[bold bright_white]Instance Segmentation Module[/bold bright_white]", style="bold white")
                ins_segmentation.run_interactive()

            elif choice == "0":
                console.print("\n[bold bright_red] Exiting Neuron Segmentation CLI... [/bold bright_red]\n")
                break
